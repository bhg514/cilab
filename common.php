<?php
	...
	function while_get_production_list($start_num,$name,$category,$status){
		...
		$query = 'SELECT @ROWNUM := @ROWNUM + 1 AS row, p.* FROM tb_product p, (SELECT @ROWNUM := '.$start_num.') R where p.fd_name like "%'.$name.'%" and p.fd_category like "%'.$category.'%" and p.fd_status like "%'.$status.'%" limit '.$start_num.', 10';		
		$result = query_send($query);		
		return $result;
	}
	...
	function product_dell($arr){
		$arr2str = implode(',', $arr);
		$query = 'delete from tb_product where pk_no in ('.$arr2str.')';
		query_send_non_return($query);
	}
	...
	function while_get_order_list($start_num,$order_number,$order_name,$order_no,$status){
		$start_num = ($start_num-1)*10;		
		if($order_no == null){
			$query = 'SELECT @ROWNUM := @ROWNUM + 1 AS row, o.* FROM tb_order o, (SELECT @ROWNUM := '.$start_num.') R where o.fk_order_number like "%'.$order_number.'%" and o.fd_order_name like "%'.$order_name.'%" and fd_status like "%'.$status.'%" limit '.$start_num.', 10';					
		}else{
			$query = 'SELECT @ROWNUM := @ROWNUM + 1 AS row, o.* FROM tb_order o, (SELECT @ROWNUM := '.$start_num.') R where o.pk_no in ('.$order_no.') limit '.$start_num.', 10';					
		}
		$result = query_send($query);		
		return $result;
	}
